...
const createWork = asyncHandler(async (req, res) => {
  const { particulars, type, size, party, dateAndTime, quantity, rate, currency = "INR", paid = false } = req.body;
  ...
  try {
    ...
    res.status(201).json({
      ...
      data: {
        ...
        dateAndTime: moment(work.dateAndTime).tz("Asia/Kolkata").format("YYYY-MM-DD HH:mm:ss"),
    ...
    if (error.name === "ValidationError") {
      throw createError(400, `Validation failed: ${Object.values(error.errors).map(e => e.message).join(", ")}`);
    }
  ...
  res.json({
    ...
    data: {
      docs: works.docs.map(doc => ({
        ...
        party: doc.party,
        partyId: doc.partyId,
        dateAndTime: moment(doc.dateAndTime).tz("Asia/Kolkata").format("YYYY-MM-DD HH:mm:ss"),
